 ...
 * @param {*} points - Set of graph's points
 * @param {*} startId - Edge's start index
 * @param {*} endId - Edge's end index
 * @param {string} label - Edge's type
 * @param {string} symbols - Symbols that are represented by this edge
 * @returns Elements in a <svg> tag like <line>,<elipse>,<polygon>,<text>,...
 */
 const Edge=({points,startId,endId,label,symbols})=>{
    const startPoint= points.filter(point=>(point.id===startId))[0];
    const endPoint= points.filter(point=>(point.id===endId))[0];
